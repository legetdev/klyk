...
- **autonomous** (default for every new session) — invisible-first for **native apps**, but the degree of invisibility differs by event type, confirmed empirically (2026-07-02) against independent native apps (TextEdit, Finder):
  - **Keyboard (`CGEventPostToPid`) and scroll (`SLEventPostToPid`)** are genuinely invisible **regardless of frontmost state** — no activation is ever needed or performed. Scroll matches the same macOS convention as a real trackpad gesture over a background window (content updates without bringing it forward); keyboard is PID-targeted and never depends on key-window status.
  - **Click, double/triple-click, and drag** require the target to already be the frontmost/key app for the delivered event to have any effect. This is **not** an over-cautious klyk safety margin — a raw SkyLight click into a non-key native window posts without error but silently does nothing (no caret move, no menu/sidebar action), matching AppKit's standard "first click brings the window forward, doesn't interact" behavior. So if the target isn't already frontmost, klyk activates it, posts, and writes the escalation to `session.escalation_log` (capped at 500 entries, FIFO eviction) — expect this to fire on essentially every first click after a context switch, not as a rare edge case.
...
- ✅ `click(x, y, button, modifiers)` — full seamless coverage including modifier-flag stamping. Cmd+click → open-in-new-tab, Shift+click → range select, etc. all land invisibly. (Phase 2 + 2.5)
...
- ✅ `drag(x1, y1, x2, y2, button, modifiers)` — stamped mouse-down → ~20 interpolated stamped dragged events → stamped mouse-up. Modifier flags held across the whole sequence. (Phase 2.5)
- ✅ `scroll(x, y, direction, amount, modifiers)` — stamped scroll-wheel event routed to the target PID. Vertical uses the wheel1 ctor arg; horizontal stamps `kCGScrollWheelEventDeltaAxis2`. Cmd+scroll (zoom) and Shift+scroll (horizontal in some apps) supported via modifier flags. Primer skipped — scroll-wheel events don't share the Chromium renderer-trust filter that clicks hit. **Also exempt from the frontmost/activation check** that click-family actions require (fixed 2026-07-02): confirmed empirically that a backgrounded native-app scroll moves the visible content with zero activation, so `_seamless_post` posts it directly regardless of frontmost state instead of unnecessarily bringing the app forward first. (Phase 2.5)
...
- ✅ `fill_field(x, y, text)` — three-stage cascade: (1) `ax_set_value` — pure AXUIElementSetAttributeValue write, no cursor / keyboard / clipboard side effects, native macOS text inputs only (web inputs detected via AXWebArea ancestor walk and skipped); (2) SkyLight focus-click + Cmd+A + paste; (3) cursor-warp focus-click + Cmd+A + paste. Response carries `via: 'ax_set_value' | 'skylight' | 'cursor_warp'` and an `ax_skip_reason` when the AX path bailed (web_backed, not_text_input, not_settable, no_element). (Phase 2 + 2.5)
- ✅ `screenshot` — skips `activate_app` and `AXRaise` in seamless modes (capture is z-order independent regardless). (Phase 2)
- ✅ Keyboard tools (`press_key`, `type_text`, `hold_key`) — `CGEventPostToPid` keystrokes are invisible for native apps. On Chromium-based apps the renderer drops keydowns to a background window, so autonomous activates the app first (background bails with `requires_foreground`); `type_text` defaults to real per-char keys on Chromium-based apps since clipboard paste is ignored by keydown-driven web UIs. (Phase 2 + later)

**Response contract:** every click-class response in seamless modes includes a `via` field — `"skylight"`, `"skylight+primer"`, `"skylight+activated"`, `"ax_set_value"`, `"ax_action"` (with `action` = which AX action fired and `level` = "element" / "parent_1" / "parent_2"), `"ax_match+skylight"` (+`"+primer"` for Chromium — AX matched the label but no AX action worked, so SkyLight delivered at the matched coords), `"ocr+skylight"`, or `"cursor_warp"` — so the agent always knows which delivery path ran. Background-mode failures return `{ok: false, requires_foreground: true, reason: <specific>, suggestion: <next step>}`. Autonomous-mode escalations carry `escalated_from: <error_token>` on the cursor-warp response so the agent (or a later human review of `get_escalation_log`) can tell escalation cursor-warps apart from humanoid-mode cursor-warps. The token is either a genuine SkyLight failure (e.g. `activation_failed`) or — for Chromium-based apps — `chromium_cursor_warp`, meaning klyk deliberately chose the real cursor because the renderer can't be trusted with synthetic clicks.
...
### 5. Maturity caveat — not re-verified end-to-end

klyk is early and experimental. Tools have been statically audited and individually live-tested, but the codebase has **not** been re-verified end-to-end on the current tree after the most recent shared-path changes (including removing the HTTP bridge). Treat it as unproven: it works in the cases its author has tested, and any path that hasn't fired against a real app should be assumed unverified.
