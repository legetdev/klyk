...
- **autonomous** (default for every new session) attempts invisible input for native apps:
  - Plain keys use PID-targeted `CGEventPostToPid`; scroll uses window-targeted SkyLight delivery. Their successful native paths do not activate the app.
  - Native clicks and same-app drags attempt `skylight.make_window_key` before posting. This can make controls respond without raising the window or changing the frontmost app. A `via` value reports the route attempted, not independent proof that the intended control changed. Unsupported delivery may require a visible fallback.
  - Command shortcuts and clipboard paste require the active app's menu handling. Menus, dialogs, hover, long presses, and cross-app drags can also activate or use visible input. Activation and explicit-window focus must succeed before the dependent input is sent.
...
**Delivery-aware tools:** native click, double/triple-click, scroll, and same-app drag attempt stamped SkyLight events. `click_element` first tries supported native AX actions, then grounded input; Chromium uses AX for location and the foreground mouse route for delivery. `fill_field` tries an AX value write before its activation-dependent click, select-all, and paste fallback. Keyboard tools use PID delivery, with activation required for Chromium, command shortcuts, and paste. Background mode refuses required activation or visible input. Long presses, hover, menus, dialogs, and cross-app drags have additional visible-input requirements.

**Response contract:** `via` identifies the selected route, such as `skylight+keyed`, `ax_action`, `ax_set_value`, or `cursor_warp`; it does not prove the app completed the task. Required foreground delivery is refused in background mode. Autonomous visible fallbacks include an escalation reason when available. Observe the resulting control, document, or independent application state before continuing an uncertain workflow.
...
`inspect` returns the image plus a filtered AX element list capped at 50 entries (`ax_elements`, `ax_element_count`, `ax_truncated` when over cap); `screenshot` defaults to image only. The AX read is best-effort: if it fails, `ax_elements=[]` and `ax_error` is set — the screenshot itself never fails. Canonical truth: the image is the source for visual state; the AX list is a reference for label-based targeting.

The AX walk dominates observation latency. On Chromium with `--force-renderer-accessibility` (or any heavy Electron tree) the AX walk takes ~1.3–1.9 s; the capture path itself (`take_screenshot` + ImageIO encode) is roughly 50–150 ms, the rest is AX traversal + filter. Two distinct tools encode this trade-off so the agent doesn't have to remember a flag: `inspect` returns image + AX list (the default for unknown or visual UI); `screenshot` returns image only for pure-visual cases (diagnostics, UI evaluation, before/after captures). The tool name carries the intent, removing one opt-in/opt-out from the agent's decision surface.
...
Every tool call logs its name and argument keys (not argument values) on entry and `done: <name> duration_ms=N gap_ms=N` on exit (gap = wall time since the previous top-level call returned, approximating LLM reasoning time). Nested calls inside `run` are tagged `nested=1` and don't update the gap anchor. Top-level responses get a `_meta: {duration_ms, gap_ms}` block injected into the trailing JSON payload. Logs roll at 10 MB, 5 backups, in `~/klyk.log`.
...
**What depends on it.** The invisible-input differentiator. The `autonomous` (default) and `background` modes attempt native click, scroll, and drag delivery through SkyLight's private `SLEventPostToPid` / `SLEventSetWindowLocation` symbols. Delivery exceptions can require activation or visible input; background refuses those paths. Keyboard input does **not** depend on SkyLight — it uses the public `CGEventPostToPid`.
...
Chromium can acknowledge AX actions or synthetic background clicks without delivering the intended page action. `click_element` uses AX to locate the target but selects the foreground mouse route before sending input; it does not issue an AX action and retry it afterward. Native semantic controls retain the AX-first path. Chromium keys and clipboard paste also require a frontmost app. Background mode refuses required activation or visible input.

Invisibility is a delivery option, not a guarantee for every native control. Menus, system dialogs, long presses, hover interactions, and cross-app drags can need visible input; command shortcuts and clipboard paste need the active app. For browser DOM work, prefer an available dedicated browser driver, with klyk for native chrome/dialogs or the documented browser fallback.
